...
//                 continue;
//             case Token::Kind::INF:
//                 expressionStack.push(calcpp::math::PosInf);
//                 continue;
//             case Token::Kind::BIN:
...
//                     THROW_ERROR("Function '" << token->view() << "' found no
//                     argument");
//                 }
...
//                     << token->view());
//             }
//             if (operators::isSingle(token->kind)) {
//                 switch (token->kind) {
...
//                         expressionStack.push(varargs(expressionStack.pop()));
//                         break;
//                     default:
//                         THROW_ERROR("Unsupported Unary Operator: " << token->view());
//                 }
//             } else {
//                 if (expressionStack.stackSize() < 2) {
//                     THROW_ERROR(
...
//                     isPostImplicit(std::next(current)->kind)) {  // Implies that the
//                     ID
//                                                                  // is a expression
//                                                                  call
...
//                     if (operatorStack.peek()->kind == LPAREN ||
//                         operatorStack.peek()->kind == LBRACE ||
//                         operatorStack.peek()->kind == COMMA) {
...
//                     }
//                     if (operatorStack.peek()->kind == COMMA ||
...
//                     outputStack.push(operatorStack.pop());
//                 }
//                 if (!operatorStack.empty() && operatorStack.peek()->kind == ID) {
//                     outputStack.push(operatorStack.pop());
//                 }
//                 continue;
//             }
...
//         if (isOperator(token.kind)) {
//             if (token.kind == MINUS && (current == tokens.begin() ||
//                                         !isPreImplicit(std::prev(current)->kind))) {
//                 token.kind = CALL;
//             } else if (!operatorStack.empty()) {
//                 auto kind = operatorStack.peek()->kind;
//                 while ((kind == CALL ||
//                         (isOperator(kind) && operators::precedence(kind) >
//                                                  operators::precedence(token.kind))
//                                                  ||
//                         (isOperator(kind) &&
//                          operators::precedence(kind) ==
//                              operators::precedence(token.kind) &&
//                          !isRightAssociative(token.kind))) &&
//                        kind != LPAREN) {
//                     outputStack.push(operatorStack.pop());
//                     if (operatorStack.empty()) break;
//                     kind = operatorStack.peek()->kind;
//                 }
//             }
//             operatorStack.push(&token);
//         } else {
//             cout << "TOKENS:  ";
//             for (auto& t : tokens) { cout << t.view() << " "; }
//             cout << endl;
//             THROW_ERROR(
//                 "Unsupported Token: '" << token.view() << "' of kind " <<
//                 token.kind);
//         }
//     }

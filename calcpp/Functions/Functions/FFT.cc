...
#define MAKE_UNIQUE_GSL(F)                                                             \
    ...
    namespace fexpr {
        ...
        struct fft: public FunctionExpression {
            fft() : FunctionExpression("fft", {arg("m")}) {}
            expression call(expression args) const override {
                auto matrix = arg->at(0);
                if (matrix == Type::MATRIX &&
                    ...
                    if (matrix == Type::COMPLEX) {
                        ...
                        for (size_t i = 0; i < N; ++i) {
                            data.get()[2 * i] =
                                GSL_REAL(gsl_matrix_complex_get(gsl_mat.get(), 0, i));
                            data.get()[2 * i + 1] =
                                GSL_IMAG(gsl_matrix_complex_get(gsl_mat.get(), 0, i));
                        }
                        ...
                        gsl_fft_complex_forward(
                            data.get(), 1, N, complextable.get(), work.get());
                        ...
                        gsl_fft_real_transform(
                            gsl_mat->data, 1, N, real.get(), work.get());
                ...
                THROW_ERROR("FFT expected 1D Matrix. Got: " << matrix);
            }
        };

        // @Function ifft
        struct ifft: public FunctionExpression {
            ifft() : FunctionExpression("ifft", {arg("m")}) {}
            expression call(expression args) override {
                ...
                if (matrix == Type::MATRIX &&
                    ...
                    if (matrix == Type::COMPLEX) {
                        ...
                        gsl_fft_halfcomplex_inverse(
                            data.get(), 1, N, halfcomplex.get(), work.get());
                ...
                THROW_ERROR("IFFT expected 1D Matrix. Got: " << matrix);
            }
        };
    ...
    namespace functions {
        ...
        const expression fft = std::shared_ptr<fexpr::fft>(new fexpr::fft());
        const expression ifft = std::shared_ptr<fexpr::ifft>(new fexpr::ifft());
        // end sourcegen
    }  // namespace functions
}  // namespace calcpp

...
with open(os.path.join(file_dir, "tokens.yml")) as file:
    ...
    unitconversions = tokens["unitconversions"]
    ...
    template.replace(
        ...
        functionOrderByLength=wrap(
            (
                str(i)
                for i, f in sorted(
                    list(enumerate(functions)), key=lambda k: len(k[1][0]), reverse=True
                )
            )
        ),
...
with Template(
    ...
    for category, specs in unitconversions["categories"].items():
        ...
        conversions.extend(
            (
                specs.get("custom", {}).get(prefix, f"GSL_CONST_NUM_{prefix.upper()}")
    ...
    unq, unq_idx, unq_cnt = np.unique(abbrs, return_inverse=True, return_counts=True)

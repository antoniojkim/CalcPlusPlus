...
def main(args):
    tokens = generate_tokens(args)
    functions = generate_functions(args)
    dists = generate_random_distributions(args)
    units = generate_unitconversions(args)

    functionNames = functions["functionNames"]

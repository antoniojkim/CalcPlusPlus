...
double IntegerDivisionExpression::value(const double& x) { return int(lhs->value(x) / rhs->value(x)); }
double IntegerDivisionExpression::value(const double& x, const double& y) { return int(lhs->value(x, y) / rhs->value(x, y)); }
